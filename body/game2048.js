document.addEventListener("DOMContentLoaded", function () {
    ...
    function move(direction) {
        ...
        for (let i = 0; i < animate1.length; i++) {
            animateMovePromise(animate1[i].from, animate1[i].dis, animate1[i].val);
        }
        for (let i = 0; i < animate2.length; i++) {
            animateMovePromise(animate2[i].from, animate2[i].dis, animate2[i].val);
        }
        for (let i = 0; i < move1.length; i++) {
            animateMove(move1[i].from, move1[i].dis, move1[i].val);
        }
        for (let i = 0; i < merge.length; i++) {
            animateMove(merge[i].from, merge[i].dis, merge[i].val);
        }
        for (let i = 0; i < move2.length; i++) {
            animateMove(move2[i].from, move2[i].dis, move2[i].val);
        }
    ...
    function animateMove(from, dis, val) {
        ...
        switch (direction) {
            ...
            case "left":
                ...
                fromSquare.style.animation = `moveLeft 0.215s forwards`;
                break;
            case "right":
